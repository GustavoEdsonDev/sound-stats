...
import { formatTopTracks, getTrackDetails } from '@/utils/spotifyFormatter';

/**
 * Buscar e formatar top tracks de forma simples
 */
async function exemplo1_Simples() {
  try {
...
async function exemplo2_Detalhado() {
  try {
    const token = getAccessToken();
    if (!token) throw new Error('Token não disponível');
...
async function exemplo4_Estatisticas() {
  try {
    const token = getAccessToken();
    if (!token) throw new Error('Token não disponível');
...
// ============================================
// EXEMPLO 5: Com Tratamento de Erros Robusto
// ============================================

async function exemplo5_Robusto() {
  try {
    ...
    // 2. Verificar se está próximo de expirar
    import { isTokenNearExpiration } from '@/utils/auth';
    if (isTokenNearExpiration()) {
      console.warn('⚠️ Token expirando em menos de 5 minutos!');
    }
...
3. Para usar em componente:
   import { TopTracksExample } from '@/app/examples/top-tracks-example';
   <TopTracksExample />
